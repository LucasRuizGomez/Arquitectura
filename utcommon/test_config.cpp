...
#include <gtest/gtest.h>  // Incluir Google Test
#include <fstream>

void cleanUpTestFiles() {
    std::remove("valid_config.cfg");
    std::remove("invalid_value.cfg");
    std::remove("defaults.cfg");
    std::remove("comments_empty.cfg");
    std::remove("invalid_fov.cfg");
    std::remove("invalid_material_rng.cfg");
    std::remove("invalid_ray_rng.cfg");
    std::remove("camera_defaults.cfg");
    std::remove("multiple_camera_values.cfg");
    std::remove("invalid_aspect_ratio.cfg");    
}
...
TEST(ConfigTest, ReadConfig_DefaultValues) {
    ...
    EXPECT_EQ(cfg.image_width, 1920);   // Valor por defecto
...
TEST(ConfigTest, ReadConfig_IgnoreCommentsAndEmptyLines) {
    ...
    EXPECT_EQ(cfg.image_width, 1024);  // Solo se lee este parámetro
}
