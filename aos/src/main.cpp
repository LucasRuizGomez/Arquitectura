...
namespace render {
  ...
  // Ahora devuelve un render::vector
  ...
  // --- HELPER PARA ESCRIBIR COLOR ---
  // Ahora acepta un render::vector
  static void write_color(ImageAOS & image, int x, int y, vector color) {
  ...
  class Camera {
  public:
    Camera(vector lookfrom, vector lookat, vector vup,  // <-- usa vector
  ...
  private:
    vector origin, lower_left_corner, horizontal, vertical;  // <-- usa vector
    vector u, v, w;                                          // <-- usa vector
  };

  // --- CONTEXTO DE RENDERIZADO ---
  struct RenderContext {
    vector bg_dark;   // <-- usa vector
    vector bg_light;  // <-- usa vector
  };

  // --- FUNCIÓN PRINCIPAL DE COLOR ---
  vector ray_color(  // <-- usa vector
      Ray const & r, Scene const & scene, RenderContext const & ctx) {
...
int main(int argc, char * argv[]) {
  try {
    ...
    render::vector cam_pos    = render::parse_vector_from_string(cfg.camera_position);
    render::vector cam_target = render::parse_vector_from_string(cfg.camera_target);
    render::vector cam_north  = render::parse_vector_from_string(cfg.camera_north);

    render::RenderContext ctx;
    ctx.bg_dark  = render::parse_vector_from_string(cfg.background_dark_color);
    ctx.bg_light = render::parse_vector_from_string(cfg.background_light_color);

    render::Camera camera(cam_pos, cam_target, cam_north, cfg.field_of_view, aspect_ratio);
    ...
    for (int y = 0; y < height; ++y) {
      ...
      for (int x = 0; x < width; ++x) {
        ...
        render::Ray r              = camera.get_ray(u, v);
        render::vector pixel_color = render::ray_color(r, scene, ctx);
        render::write_color(image, x, y, pixel_color);
      }
    }
